...
import { recordPayment } from "@/lib/credit";
...
function NewSale() {
  ...
  const { data: restaurants } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("restaurants").select("id,name").eq("is_active", true).order("name"))
        .data ?? [],
  });
  const { data: products } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("products")
          .select("id,name,unit,sku,current_stock,avg_cost,categories(name)")
          .order("name")
      ).data ?? [],
  });
    ...
    setLines(
      lines.map((l, idx) => {
        ...
        if (next.price_mode === "percent") {
          next.unit_price = Number(
            (next.cost * (1 + (Number(next.markup_percent) || 0) / 100)).toFixed(2),
          );
        }
        return next;
      }),
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (lines.some((l) => !l.unit_price || l.unit_price <= 0))
        throw new Error("Enter a selling price for every item");
      if (hasOverstock) throw new Error("One or more items exceed available stock");
      if (received > 0 && !vaultUserId)
        throw new Error("Select the Vault that received this payment");
      ...
      const { data: saved } = await supabase
        ...
        .eq("sale_id", sale.id);
      const totalCost = (saved ?? []).reduce(
        (s, x) => s + Number(x.quantity) * Number(x.cost_price),
        0,
      );
                ...
                <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-lg border border-border bg-popover shadow-lg">
                  {results.map((p: any) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs ${Number(p.current_stock) <= 0 ? "text-destructive" : "text-muted-foreground"}`}
                      >
                      ...
                      <TableRow key={l.product_id}>
                        <TableCell className="font-medium">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Available: {formatNumber(l.stock)} {l.unit}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Input
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            {l.price_mode === "amount" ? (
                              ...
                              <Input
                                ...
                                className="w-24"
                                placeholder="%"
                                value={l.markup_percent || ""}
                                onChange={(e) => updateLine(i, { markup_percent: +e.target.value })}
                              />
                            )}
                            <Select
                              value={l.price_mode}
                              onValueChange={(v) => updateLine(i, { price_mode: v as any })}
                            >
                          ...
                          {l.cost > 0 && (
                            <div className="mt-1 text-xs text-muted-foreground">
                              ...
                              {l.price_mode === "percent" && l.unit_price > 0 && (
                                <> · Price: {formatCurrency(l.unit_price)}</>
                              )}
                            </div>
                          )}
                        </TableCell>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {discountMode === "percent" && subtotal > 0 && (
                <p className="text-xs text-muted-foreground">
                  = {formatCurrency(discount)} off {formatCurrency(subtotal)}
                </p>
              )}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Leave 0 to record the full amount as credit.
              </p>
            </div>
            {received > 0 && (
              <div className="space-y-2">
            ...
            <div className="space-y-1 border-t border-border pt-4 text-sm">
              ...
              <div className="flex justify-between text-success">
                <span>Received</span>
                <span>{formatCurrency(Math.min(received, grandTotal))}</span>
              </div>
              <div className="flex justify-between font-semibold text-destructive">
                <span>Balance (credit)</span>
                <span>{formatCurrency(Math.max(0, grandTotal - received))}</span>
              </div>
            </div>
            <Button
              className="w-full"
              onClick={() => save.mutate()}
              disabled={save.isPending || hasOverstock}
            >
