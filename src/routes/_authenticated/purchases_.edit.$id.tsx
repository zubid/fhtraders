...
import { updateSupplierPayment, PAYMENT_METHODS, METHOD_LABELS } from "@/lib/supplier-credit";
...
function EditPurchase() {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("products")
          .select("id,name,unit,sku,current_stock,categories(name)")
          .order("name")
      ).data ?? [],
  });
    ...
    queryFn: async () =>
      ((
        await (supabase.from("supplier_payments" as any) as any)
          .select("id,amount,method,vault_user_id,payment_date,note")
          .eq("purchase_id", id)
      ).data ?? []) as any[],
  });
  ...
  useEffect(() => {
    ...
    setLines(
      (purchase.purchase_items ?? []).map((it: any) => ({
  ...
  useEffect(() => {
    if (paySplits) setPaymentSplits(paySplits.map((p: any) => ({ ...p })));
  }, [paySplits]);
  ...
  const addLine = (p: { id: string; name: string; unit: string }) => {
    ...
    setLines([
      ...lines,
      { product_id: p.id, name: p.name, unit: p.unit, quantity: 1, unit_price: 0 },
    ]);
  };
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (dErr) throw dErr;
      // Update purchase
      const { error: uErr } = await (supabase.from("purchases") as any)
        .update({
      ...
      for (const split of paymentSplits)
        await updateSupplierPayment(split.id, {
          supplierId,
          purchaseId: id,
          amount: Number(split.amount),
          method: split.method,
          date: split.payment_date,
          note: split.note ?? "",
          vaultUserId: split.vault_user_id || undefined,
        });
    },
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      <PageHeader
        title={`Edit Purchase ${purchase?.reference_no ?? ""}`}
        description="Modify items or details. Stock and payment status will be recalculated."
      />
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Existing Payment Splits</Label>
              {paymentSplits.length === 0 ? (
                <p className="text-xs text-muted-foreground">No payment cash transactions.</p>
              ) : (
                paymentSplits.map((p: any, i: number) => (
                  <div key={p.id} className="rounded-md border p-2 space-y-2">
                    ...
                    />
                    <Select
                      value={p.vault_user_id ?? "none"}
                      onValueChange={(v) =>
                        setPaymentSplits(
                          paymentSplits.map((x, j) =>
                            j === i ? { ...x, vault_user_id: v === "none" ? null : v } : x,
                          ),
                        )
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Paid from vault" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none">No vault attribution</SelectItem>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setPaymentSplits(
                          paymentSplits.map((x, j) => (j === i ? { ...x, method: v } : x)),
                        )
                      }
                    >
                      <SelectTrigger>
            ...
            <p className="text-xs text-muted-foreground">
              Payments already recorded stay linked. Balance recalculates automatically.
            </p>
            {(paySplits ?? []).length > 0 && (
              <p className="text-xs text-muted-foreground">
                {(paySplits ?? []).length} payment{(paySplits ?? []).length > 1 ? "s" : ""}{" "}
                totalling{" "}
                {formatCurrency(
                  (paySplits ?? []).reduce((s: number, p: any) => s + Number(p.amount), 0),
                )}{" "}
                recorded for this purchase. Each cash split keeps its own Vault attribution and
                method.
              </p>
            )}
